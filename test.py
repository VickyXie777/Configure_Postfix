...
def getContent(resouce, final):
    ...
    try:
        for line in f:
            if yes_date in str(line):
                if "ERROR" in str(line):  # 按行读取，如果该行包含“ERORR”字符串，则将该行写入目标文件
...
if __name__ == '__main__':
    sourcedir = "/Users/apple/PycharmProjects/pythonProject1/error.txt"  # 需要读取的源文件路径
    objectdir = "/Users/apple/PycharmProjects/pythonProject1/sent.txt"  # 存放的目标文件
    mailto_list = ["###@qq.com"]  # 收件人邮箱，可以发送存放多个
